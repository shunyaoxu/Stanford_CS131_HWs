...
def rotate2d(point, theta):
    ...
    out[1] = point[1]*np.cos(theta) + point[0]*np.sin(theta)
    return out
    ### END YOUR CODE
...
def rotate_image(input_image, theta):
    ...
    ## YOUR CODE HERE
    point = np.zeros(shape=(2,))
    for i in range(input_rows):
        for j in range(input_rows):
            point[0] = i - input_rows/2
            point[1] = j - input_cols/2
            point = rotate2d(point, theta)
            point[0] = point[0] + input_rows/2
            point[1] = point[1] + input_cols/2
            if int(point[0])>=0 and int(point[0])<input_rows and int(point[1])>=0 and int(point[1])<input_cols:
