...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        A = np.concatenate([A_Iy, A_Ix], axis=1)
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        G_Iy = Iy[y1-w:y1+w+1, x1-w:x1+w+1].reshape(-1,1)
        G_Ix = Ix[y1-w:y1+w+1, x1-w:x1+w+1].reshape(-1,1)
        G = np.array([[np.sum(G_Ix**2),np.sum(G_Ix*G_Iy)],[np.sum(G_Ix*G_Iy),np.sum(G_Iy**2)]]).reshape(2,2)
        ### END YOUR CODE
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            Ik = (img1[y1-w:y1+w+1, x1-w:x1+w+1] - img2[y2-w:y2+w+1, x2-w:x2+w+1]).reshape(-1,1)
            bk = np.array([[np.sum(Ik*G_Ix)],[np.sum(Ik*G_Iy)]]).reshape(2,1)
            vk = np.dot(np.linalg.inv(G), bk).ravel()
            ### END YOUR CODE
...
def pyramid_lucas_kanade(
    ...
    for L in range(level, -1, -1):
        ...
        d = iterative_lucas_kanade(pyramid1[L], pyramid2[L], P_L, window_size=window_size, num_iters=num_iters, g=g)
        if L != 0:
            g = scale * (g + d)
        ### END YOUR CODE
...
def compute_error(patch1, patch2):
    ...
    error = 0
    ### YOUR CODE HERE
    patch1Norm = (patch1 - np.mean(patch1)) / np.std(patch1)
    patch2Norm = (patch2 - np.mean(patch2)) / np.std(patch2)
    error = np.mean((patch1Norm - patch2Norm) * (patch1Norm - patch2Norm))
    ### END YOUR CODE
    return error
...
def IoU(bbox1, bbox2):
    ...
    if x1 >= x2:
        inters_x_left = x1
        if x1+w1 <= x2+w2:
            inters_x_right = x1+w1
        else:
            inters_x_right = x2+w2
    else:
        inters_x_left = x2
        if x1+w1 <= x2+w2:
            inters_x_right = x1+w1
    ...
    if y1 >= y2:
        inters_y_left = y1
        if y1+h1 <= y2+h2:
            inters_y_right = y1+h1
        else:
            inters_y_right = y2+h2
    else:
        inters_y_left = y2
        if y1+h1 <= y2+h2:
            inters_y_right = y1+h1
    ...
    intersection = (inters_x_right-inters_x_left)*(inters_y_right-inters_y_left)
