...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    wIx2 = convolve(dx*dx, window)
    wIy2 = convolve(dy*dy, window)
    wIxy = convolve(dx*dy, window)
    response = (wIx2 * wIy2 - wIxy * wIxy) - k * (wIx2 + wIy2) ** 2
    ### END YOUR CODE
...
def simple_descriptor(patch):
    ...
    if std == 0:
        std = 1
    normalizedPatch = (patch - mean) / std
    feature = normalizedPatch.flatten()
    ### END YOUR CODE
    return feature
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(M):
        ...
        if dist_sort[0]/dist_sort[1] < threshold:
            j = np.argmin(dist)
            matches.append([i,j])
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        index = np.random.randint(N, size=n_samples) # Select random set of matches
        sample1, sample2 = matched1[index], matched2[index]
        H = np.linalg.lstsq(sample2, sample1, rcond=None)[0] # Compute affine transformation matrix
        H[:, 2] = np.array([0, 0, 1])
        new_inliers = np.sum(np.sqrt((matched2.dot(H) - matched1) ** 2), axis=-1) < threshold # Compute inliers via Euclidean distance
        new_n_inliners = np.sum(new_inliers)
        if new_n_inliners > n_inliers: # Keep the largest set of inliers
            n_inliers = new_n_inliners
            max_inliers = new_inliers
    H = np.linalg.lstsq(matched2[max_inliers], matched1[max_inliers], rcond=None)[0] # Re-compute least-squares estimate
    H[:,2] = np.array([0, 0, 1])
    ### END YOUR CODE
        ...
        for j in range(cols):
            for k in range(cell_rows):
                for m in range(cell_cols):
                    t = theta_cells[i,j,k,m]
                    g = G_cells[i,j,k,m]
                    index = int(t / degrees_per_bin)
                    if(index == 9):
                        index = 0;
                    hists[i, j, index] += g
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    weight1 = np.ones(img1_warped.shape)
    weight2 = np.ones(img2_warped.shape)
    weight1[:, left_margin:right_margin] = np.linspace(1, 0, right_margin-left_margin)
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    H = []
    for i in range(len(imgs)-1):
        H.append(ransac(keypoints[i], keypoints[i+1], matches[i], threshold=1)[0])
    
    output_shape, offset = get_output_space(imgs[1], [imgs[0],imgs[2],imgs[3]], [np.linalg.inv(H[0]), H[1], H[1].dot(H[2])])
