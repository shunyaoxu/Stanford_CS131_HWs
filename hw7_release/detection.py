...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            score = 0
            ### YOUR CODE HERE
            slidingWindow = pad_image[r:r+winH, c:c+winW]
            (face_feature, slidingWindowHog) = hog_feature(slidingWindow, pixel_per_cell)
            score = np.dot(face_feature, template_feature)
            if score > max_score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    for scale, image in images:
        max_score_cur, maxr_cur, maxc_cur, response_map_cur = sliding_window(image, template_feature, step_size, shape, pixel_per_cell=pixel_per_cell)
        if max_score_cur > max_score:
            max_score = max_score_cur
            maxr = maxr_cur
            maxc = maxc_cur
            max_scale = scale
            max_response_map = response_map_cur
    ### END YOUR CODE
...
def detect_multiple(image, response_map, winH, winW):
    ...
    ### YOUR CODE HERE
    detected_faces = []
    response_mapC = response_map.copy()
    threshold = 3.7
    while True:
        maxr, maxc = np.unravel_index(np.argmax(response_mapC), response_mapC.shape)
        score = response_mapC[maxr, maxc]
        if score < threshold: break;
        maxr -= winH // 2
        maxc -= winW // 2
        response_mapC[maxr:maxr+winH, maxc:maxc+winW] = 0
