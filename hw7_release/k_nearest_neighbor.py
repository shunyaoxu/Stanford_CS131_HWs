...
def compute_distances(X1, X2):
    ...
    X1_2 = np.sum(np.square(X1), axis=1).reshape(-1, 1)
    X2_2 = np.sum(np.square(X2), axis=1)
    dists = np.sqrt(X1_2 - 2 * X1.dot(X2.T) + X2_2)
    # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_train_split = np.array_split(X_train, num_folds, axis=0)
    y_train_split = np.array_split(y_train.reshape(-1, 1), num_folds, axis=0)
    for i in range(num_folds):
        X_trains[i] = np.vstack(X_train_split[:i]+X_train_split[i+1:])
        y_trains[i] = np.vstack(y_train_split[:i]+y_train_split[i+1:])[:, 0]
        X_vals[i] = X_train_split[i]
        y_vals[i] = y_train_split[i][:, 0]
    # END YOUR CODE
